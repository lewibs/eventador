...
EventTarget.prototype.eventadorClasicAddEventListener = EventTarget.prototype.addEventListener;

EventTarget.prototype.eventadorClasicRemoveEventListener = EventTarget.prototype.removeEventListener;

//these are only to filter to eventador
EventTarget.prototype.removeEventListener = function(event, callback, options) {
    return this.eventadorClasicRemoveEventListener(event, callback, options);
}
...
EventTarget.prototype.addEventListener = function(event, callback, options) {
    ...
    try {
        ...
        //a react event
        if (/react-/.test(event)) {
            return this.eventadorClasicAddEventListener(event, callback, options, this);
        }

        return Eventador.addListener(event, callback, options);
    } catch (e) {
        return this.eventadorClasicAddEventListener(failedEvent, failedCallback, failedOptions);
    }
}